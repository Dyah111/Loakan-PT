...
<div class="max-w-2xl mx-auto py-8">
    ...
    <div class="bg-white p-4 rounded shadow mb-6">
        <form action="{{ route('forum.store') }}" method="POST">
            @csrf
            <textarea name="message" rows="3" class="w-full border border-gray-300 rounded p-2 mb-2" placeholder="Apa yang ingin kamu bagikan?"></textarea>
    ...
    <div class="bg-white rounded-lg p-6 shadow mb-4 relative" x-data="{ open: false }">
        ...
        <div class="absolute top-4 right-4">
            ...
            <div x-show="open" @click.away="open = false" class="mt-2 w-40 bg-white border rounded shadow absolute right-0 z-10">
                <ul class="text-sm text-gray-700">
                    <li @click="alert('Laporan terkirim')" class="px-4 py-2 hover:bg-gray-100 cursor-pointer">Laporkan</li>
                    @if ($forum->user_id === Auth::id())
                    <form action="{{ route('forum.destroy', $forum->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="w-full text-left px-4 py-2 hover:bg-red-100 text-red-600">Hapus</button>
                    </form>
                    @endif
                </ul>
            </div>
